...
function MRS.HudPlayer(ply, Draw3D)
	...
	if ply:GetPos():DistToSqr(LocalPlayer():GetPos()) < d_dist ^ 2 then
		ply.mrsW = Lerp(FrameTime() * 3, ply.mrsW, 1)
	else
		ply.mrsW = Lerp(FrameTime() * 3, ply.mrsW, 0)
	end

	if ply.mrsW <= 0.1 then return end
