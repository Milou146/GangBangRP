...
function ENT:Initialize()
    ...
    self.ShelfSlots = {
        --{item = false, edit = false, itemClass = "", price = 0, amount = 0, perm = false, offset = function(shelf) return shelf:GetAngles():Up() * 8 end, pos = Vector(0,0,0), ang = Angle(0,0,0) }, --{item = false, edit = false, itemClass = "",  price = 0, amount = 0, perm = false, offset = function(shelf) return shelf:GetAngles():Up() * 33 end, pos = Vector(0,0,0), ang = Angle(0,0,0)}, --{item = false, edit = false, itemClass = "", price = 0, amount = 0, perm = false, offset = function(shelf) return shelf:GetAngles():Up() * 59 end, pos = Vector(0,0,0), ang = Angle(0,0,0)},
        {
            item = false,
            edit = false,
            itemClass = "",
...
local function ShelfPickup(ply, ent)
    if ent:GetClass() == "weapon_shelf" then return true end
end
...
net.Receive("shelf_buyItem", function(len, ply)
    ...
    if not IsValid(shelf) or not shelf.ShelfSlots or not shelf.ShelfSlots[id] or not shelf.ShelfSlots[id].item then
        print(ply:Name() .. " is sending an id for a shelf item that doesn't exist! They maybe attempting to crash the server.")
...
function LoadShelves()
    local datatbl = util.JSONToTable(file.Read("shelf_data/shelves/shelf.txt", "DATA"))
...
local function ShelfPickup(ply, ent)
    if ent:GetClass() == "weapon_shelf" and ent.PhysgunPickupEnabled and IsValid(ent:Getowning_ent()) and ent:Getowning_ent() == ply then return true end
end
...
hook.Add("PostCleanupMap", "RespawnPermShelves", function()
    if file.Exists(GetShelfFile(), "DATA") then
        LoadShelves()
    end
end)