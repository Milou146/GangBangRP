...
timer.Create("IsBodyGrouprMenuStillValid", 3, 0, function()
    if BODYMAN.MenuIsOpen == true and BODYMAN.ClosetsOnly == true and not BODYMAN:CloseEnoughCloset(LocalPlayer()) and IsValid(BODYMAN.Menu) then
        BODYMAN.Menu:Close()
    end
end)
...
function BODYMAN:OpenMenu()
    ...
    if self.ClosetsOnly == true and self:CloseEnoughCloset(LocalPlayer()) == false then return false end
    ...
    if IsValid(BODYMAN.ClientModelPanel) then
        BODYMAN.ClientModelPanel:Remove()
    end
...
function BODYMAN:RefreshAppearanceMenu()
    ...
    for k, v in pairs(curgroups) do
        ...
        if bgroups[v.name] and not table.HasValue(bgroups[v.name], cur_bgid) then
