...
function PlayerSpawnBodygroups(ply)
    ...
    timer.Simple(1, function()
        if ply.LastJob_bodygroupr == job.command and table.HasValue(type(job.model) == "table" and job.model or {}, ply.LastPlayermodel or "") then
...
function BODYMAN:SaveClosets()
    ...
    for k, ent in ipairs(closets) do
        table.insert(self.Data.Closets, {
            pos = ent:GetPos(),
            ang = ent:GetAngles(),
            map = game.GetMap()
        })
...
function BODYMAN:LoadClosets()
    if self.Data.Closets and self.Data.Closets ~= {} then
        -- remove old closets
...
-- load closets on map load
hook.Add("InitPostEntity", "BODYMAN:LoadClosets", function()
    BODYMAN:LoadClosets()
end)
...
function BODYMAN:HasAdmin(ply)
    if IsValid(ply) and not ply:IsSuperAdmin() then return false end
