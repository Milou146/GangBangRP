...
function BODYMAN:HasBodyGroup(ply, name, idx)
    ...
    if bgid >= 0 and modelgroups[bgid + 1].submodels[idx] then
...
function BODYMAN:CloseEnoughCloset(ply)
    ...
    if tr.Entity and tr.Entity:GetClass() == "bodyman_closet" and dist <= range then return true end
...
local playerCommandLastRun = {}

function BODYMAN:PlayerCommandIsThrottled(ply, cmd)
    local key = ply:SteamID64() .. "_" .. cmd
    local now = SysTime()
    local cooldownSeconds = 0.25

    if playerCommandLastRun[key] then
        local lastRun = playerCommandLastRun[key]
        if now - lastRun < cooldownSeconds then return true end
    end

    playerCommandLastRun[key] = now
