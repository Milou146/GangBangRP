...
function ENT:Touch(ent)
	if not IsValid(ent) then return end
		if ent.ItsUpgrade then
			if (ent.upgrade == "paper" and self.PPaper == self.MaxPapers) or (ent.upgrade == "ink" and self.PColors == self.MaxColors) then return end
			if ent.Cooler then
				self:CreateCooler(ent)
...
function ENT:StartCoolerAnim()

	if not IsValid(self.PCooler) then return end
	

	if not self.PEnable then return end
	timer.Simple(2,function()
			if IsValid(self.PCooler) then
				self.PCooler:Fire('setanimation','spoil',0)
				timer.Simple(0.2,function()
					if IsValid(self.PCooler) then
						self.PCooler:Fire('setanimation','spin',0)
							self.PCooler.sound = CreateSound(self.PCooler, Sound("ambient/machines/train_wheels_overhead_loop1.wav"))
							self.PCooler.sound:SetSoundLevel(55)
							self.PCooler.sound:PlayEx(1, 100)  
					end
				end)
			end
		end)
...
function ENT:FakeCooler(mdl)
		self.PCooler:Remove()
		self.PCooler = ents.Create('prop_dynamic')
		self.PCooler:SetModel(mdl)
		self.PCooler:SetPos(self:GetPos())
		self.PCooler:SetAngles	(self:GetAngles())
		self.PCooler:SetSolid( SOLID_VPHYSICS )
		self.PCooler:Spawn()
		self.PCooler:SetParent(self)
		self.PCooler:Fire('setanimation','idle',0)
		self:StartCoolerAnim()
end



function ENT:CreateCooler(ent)
	if not cfg["AllowCooler"] then return end
	if IsValid(self.PCooler) then return end
	ent:SetSolid(0)
	...
	self.PCooler = ent
	self.PCoolerState = ""
	self:FakeCooler(ent:GetModel())
	self.CClass = ent:GetClass()
	self:UpdateVars()
end

function ENT:RemoveCooler()
	if not IsValid(self.PCooler) then return end
	self.PCoolerState = "dis"
	self:UpdateVars()
		local id = self:LookupSequence("eject")
		self:ResetSequence(id)
		self.PCooler:Remove()
		local PCooler = ents.Create(self.CClass)
		PCooler:SetPos(self:GetPos()+self:GetRight()*-25)
		PCooler:SetAngles	(self:GetAngles())
		PCooler:SetSolid( SOLID_VPHYSICS )
		PCooler:Spawn()
		if self.PCooler.sound then
			self.PCooler.sound:Stop()
		end
