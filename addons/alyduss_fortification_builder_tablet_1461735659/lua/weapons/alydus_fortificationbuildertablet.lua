...
if CLIENT then
	...
	if GAMEMODE.IsSandboxDerived then
		language.Add("SBoxLimit_fortifications", "Vous avez atteint la limite de construction.")
	end

	language.Add("Undone_Fortification", "Undone Fortification")

	language.Add("Cleanup_fortifications", "Fortifications")
	language.Add("Cleaned_fortifications", "Nettoyer toutes les fortifications")
elseif SERVER then
...
SWEP.Fortifications = {
	{name = "Sac de sable", model = "models/props_fortifications/sandbags_corner1.mdl"},
	{name = "Grand sac de sable", model = "models/props_fortifications/sandbags_corner1_tall.mdl"},
	{name = "Petit sac de sable", model = "models/props_fortifications/sandbags_corner3.mdl"},
	{name = "Sac de sable en ligne", model = "models/props_fortifications/sandbags_line1.mdl"},
	{name = "Grand sac de sable en ligne", model = "models/props_fortifications/sandbags_line1_tall.mdl"},
	{name = "Large de sable en ligne", model = "models/props_fortifications/sandbags_line2.mdl"},
	--{name = "Sandbags Line 2 Tall", model = "models/props_fortifications/sandbags_line2_tall.mdl"},
	{name = "Sac de sable mouillé", model = "models/props_fortifications/sandbags_line2b.mdl"},
	--{name = "Concrete Wall", model = "models/props_fortifications/concrete_wall001_96_reference.mdl"},
	{name = "Barrière en béton blanc", model = "models/props_c17/concrete_barrier001a.mdl"},
	--{name = "Petite barrière en béton", model = "models/props_fortifications/concrete_barrier001_96_reference.mdl"},
	--{name = "Concrete Barrier 2 Large", model = "models/props_fortifications/concrete_barrier001_128_reference.mdl"},
	{name = "Barrière en béton rayée", model = "models/jbarnes/props/concrete barricade.mdl"},
	{name = "Bidon en fer", model = "models/props_c17/oildrum001_explosive.mdl"}
	--{name = "Grillage", model = "models/props_c17/fence01b.mdl"},
				...
				if IsValid(v) and v:GetPos():Distance(LocalPlayer():GetPos()) <= 250 then
					if v:GetClass() == "prop_physics" and v:GetNWString("alydusFortificationHologramName", false) != false and not (ply:GetEyeTrace().Entity and (ply:GetEyeTrace().Entity:GetClass() == "prop_physics" or ply:GetEyeTrace().Entity:GetClass() == "alydus_destructablefortification")) then
						...
						cam.Start3D2D(pos, Angle(0, ang.y, 90), 0.10)
							draw.DrawText("", "Alydus.FortificationsTablet.Title", 0, 0, Color(255, 255, 255), TEXT_ALIGN_CENTER)
		...
		if IsValid(vm) then
			...
			cam.Start3D2D(pos - ang:Right() * 3 - ang:Forward() * 8.25 + ang:Right() * 7.4, Angle(0, ply:EyeAngles().y, ang.z) + Angle(180, 90, 160), 0.01)
				...
				if self:GetNWBool("tabletBootup", false) == false then
					draw.SimpleText("Fortifications", "Alydus.FortificationsTablet.Title", 15, 0, Color(255, 255, 255, 255), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
					...
					draw.SimpleText(" LMB: Prec. | RMB: Suiv.", "Alydus.FortificationsTablet.Subtitle", 15, 60, Color(150, 150, 150, 255), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)

					surface.SetMaterial(wrenchMat)
					surface.DrawTexturedRect(-215, 98, 40, 40)

					draw.SimpleText("      [" .. string.upper(useBind) .. "]: Créer | [" .. string.upper(reloadBind) .. "]: Détruire", "Alydus.FortificationsTablet.Subtitle", 15, 120, Color(150, 150, 150, 255), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)

					if alydusDestructableFortificationExtension then
						surface.SetMaterial(toolsMat)
						surface.DrawTexturedRect(-95, 158, 40, 40)

						draw.SimpleText("        [G]: Réparer", "Alydus.FortificationsTablet.Subtitle", 15, 180, Color(150, 150, 150, 255), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)

						surface.SetMaterial(shieldMat)
						surface.DrawTexturedRect(-237, 222, 40, 40)

						draw.SimpleText("        " .. table.Count(self.Fortifications) .. " Fortifications disponibles", "Alydus.FortificationsTablet.Subtitle", 15, 240, Color(100, 100, 100, 255), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
					else
						surface.SetMaterial(shieldMat)
						surface.DrawTexturedRect(-237, 158, 40, 40)

						draw.SimpleText("        " .. table.Count(self.Fortifications) .. " Fortifications disponibles", "Alydus.FortificationsTablet.Subtitle", 15, 180, Color(100, 100, 100, 255), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
					end
				else
					draw.SimpleText("Fortifications", "Alydus.FortificationsTablet.Title", 15, 30, Color(255, 255, 255, 255), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)

					surface.SetMaterial(refreshMat)
					surface.DrawTexturedRect(-130, 70, 40, 40)

					draw.SimpleText("       Initialisation...", "Alydus.FortificationsTablet.Subtitle", 15, 90, Color(150, 150, 150, 255), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
				end
			cam.End3D2D()
		end
	end
