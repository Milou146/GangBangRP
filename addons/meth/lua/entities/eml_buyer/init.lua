...
function ENT:AcceptInput(name, activator, caller)
    if (not self.nextUse or CurTime() >= self.nextUse) then
        if (name == "Use" and caller:IsPlayer() and (caller:GetNWInt("player_meth") == 0)) then
            ...
            caller:SendLua("local tab = {Color(1,241,249,255), [[Meth Addict: ]], Color(255,255,255), [[" .. table.Random(EML_Meth_Salesman_NoMeth) .. "]] } chat.AddText(unpack(tab))")

            timer.Simple(0.25, function()
                self:EmitSound(table.Random(EML_Meth_Salesman_NoMeth_Sound), 100, 100)
            end)
        elseif (name == "Use") and (caller:IsPlayer()) and (caller:GetNWInt("player_meth") > 0) then
            caller:addMoney(caller:GetNWInt("player_meth"))
            caller:SendLua("local tab = {Color(1,241,249,255), [[Meth Addict: ]], Color(255,255,255), [[" .. table.Random(EML_Meth_Salesman_GotMeth) .. " Here, you get ]], Color(128, 255, 128), [[$" .. caller:GetNWInt("player_meth") .. ".]] } chat.AddText(unpack(tab))")
            caller:SetNWInt("player_meth", 0)

            if (EML_Meth_MakeWanted) then
                caller:wanted(nil, "Drug Manufacturing")
            end

            timer.Simple(0.25, function()
                self:EmitSound(table.Random(EML_Meth_Salesman_GotMeth_Sound), 100, 100)
            end)
