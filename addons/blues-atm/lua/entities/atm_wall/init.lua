...
function ENT:IsPlayerAuthenticated(ply)
    ...
    if ply.batmLastMachineUsed ~= nil and ply.batmLastMachineUsed ~= self then return false, "anouthermachineactive" end
    --Is someone else using this machine?
    if self.currentUser ~= nil and self.currentUser ~= ply then return false, "someonealreadyusing" end
    --We are already using this machine, so we can continue to use it
    if self.currentUser == ply then return true, "theyareusing" end
    if self.currentUser == nil then return false, "nouser" end
    --Uh oh, better return false just incase something slips through
