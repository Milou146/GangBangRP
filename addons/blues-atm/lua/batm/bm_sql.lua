...
function SQL.Initialize()
    if not BATM.Config.UseMySQL then
        ...
        db = mysqloo.connect(BATM.Config.MySQLDetails.host, BATM.Config.MySQLDetails.username, BATM.Config.MySQLDetails.password, BATM.Config.MySQLDetails.databasename, tonumber(BATM.Config.MySQLDetails.port))
        local worked = false
...
function SQL.SavePersonalAccount(account)
    if not BATM.Config.UseMySQL then
        local result = sql.Query([[INSERT OR REPLACE INTO batm_personal_accounts VALUES (]] .. account.ownerID .. [[ , ]] .. sql.SQLStr(util.TableToJSON(account)) .. [[);]])
        ...
        --MySQL stuff here 
        local query = SQL.MySQLDB:query([[INSERT INTO batm_personal_accounts (steamid, accountinfo) VALUES (]] .. account.ownerID .. [[ , ']] .. SQL.MySQLDB:escape(util.TableToJSON(account)) .. [[') ON DUPLICATE KEY UPDATE accountinfo = ']] .. SQL.MySQLDB:escape(util.TableToJSON(account)) .. [[']])
...
function SQL.SaveGroupAccount(account)
    if not BATM.Config.UseMySQL then
        ...
        local result = sql.Query([[INSERT OR REPLACE INTO batm_group_accounts VALUES (]] .. account.ownerID .. [[ , ]] .. sql.SQLStr(util.TableToJSON(account)) .. [[);]])
        ...
        account.ownerNames = newOwnerNames
        local query = SQL.MySQLDB:query([[INSERT INTO batm_group_accounts (steamid, accountinfo) VALUES (]] .. account.ownerID .. [[ , ']] .. SQL.MySQLDB:escape(util.TableToJSON(account)) .. [[') ON DUPLICATE KEY UPDATE accountinfo = ']] .. SQL.MySQLDB:escape(util.TableToJSON(account)) .. [[']])
